...
from config import COLUMNS_DEFAULTS, ROW_SELECTION, DATA_PATH_FREEZER
from __version__ import version

# Global DataFrame
dg = load_dataframe(DATA_PATH_FREEZER)
...
def index_page():
    ...
    async def sync_grid_to_df():
        ...
        if client_data:
            dg = pd.DataFrame(client_data)
            print(f"Synced {len(dg)} rows from grid to DataFrame")
    
    async def save_df():
        """Save DataFrame to CSV"""
        await sync_grid_to_df()
        if save_dataframe(dg, DATA_PATH_FREEZER):
    ...
    async def delete_selected_rows():
        ...
        dg = delete_rows_by_ids(dg, selected_ids)
    ...
    with ui.column().classes('w-full p-4'):
        ui.label("Your Freezer Items").classes('text-h5 q-mb-md')
        
        # Data grid
        row_data = dg.to_dict('records')
